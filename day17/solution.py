...
class Probe():
    ...
    def check_hit(self, area):
        if self.position[0] in range(area["x"][0], area["x"][1] + 1) and self.position[1] in range(area["y"][0], area["y"][1] + 1):
...
for i in range(-1000, 1001): # range(abs(target_area["y"][0]) + 1): you know what just make the range big
